...
Because N_a_new are the wt individuals and newly mutated individuals are part of N_A_new leaving the wt pop
* In the life cycle as defined above, do individuals reproduce first and then mutate, or vice versa?
They first reproduce and then mutate
...
because you have a mutation rate of 0.01 -> 1% and a starting population of 100 (rather unrealistic mutation rate, but good for visualizing)
* What does the function `c` do?
It combines values into a vector or list
...
pop_new: output of one generation vector, Pop_vector: Whole generation -> Adds the pop_new to the value we had before
* What is the condition that breaks the loop good for? What are its disadvantages?
either to population reaches 1.5x the initial pop or it goes extinct -> good to safe computing time
* What does the function `tail` do?
returns the last parts of an object -> See what happens at the end
...
it does the process for every combination of parameters 100 times
...
we can change the value at the beginning and do not have to change the whole code -> Often the values are not only once in the code.
* What does the function `which` do?
isolates values for which (total_size==min_size)[1] is true and makes it numeric (as.numeric)
