...
results_df <- data.frame(array(NA, dim = c(length(slim_out_files),4), dimnames = list(c(),c("m","reps","seed", "Heterozygosity"))))
...
for(i in 1:length(slim_out_files)){ ## loop over all SLiM results files
  ...
  data_Fst <- data.frame(array(NA, dim = c(length(slim_out_files),3), dimnames = list(c(),c("Generations","reps","seed"))))
  ...
  ##CReate a new column with allele frequencies (Add allele freq. in a seperate row (for each file))
   single_file_df$Allelefreq <- single_file_df[,13]/50
  ...
  #Also calculate H between demes (dxy)
  ...
  results_df[i,"Fst"] <- metric ## record your metric of choice
  
  
  #first column: pop. identifier, next ones: Genotypes at each of the loci. 
